...
static inline int is_kmmio_active(void)
{
	if (kernel_init_done == 1) printk("We reached unpopular paths in include/linux/mmiotrace.h: line 60 \n"); 
	return 0;
}

static inline int kmmio_handler(struct pt_regs *regs, unsigned long addr)
{
	if (kernel_init_done == 1) printk("We reached unpopular paths in include/linux/mmiotrace.h: line 66 \n"); 
	return 0;
}
