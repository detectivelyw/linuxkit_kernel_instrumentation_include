...
{
	if (kernel_init_done == 1) printk("We reached unpopular paths in include/net/netfilter/nf_conntrack_timeout.h: line 33 \n"); 
...
struct nf_conn_timeout *nf_ct_timeout_ext_add(struct nf_conn *ct,
					      ...
					      gfp_t gfp)
{
	if (kernel_init_done == 1) printk("We reached unpopular paths in include/net/netfilter/nf_conntrack_timeout.h: line 58 \n"); 
#ifdef CONFIG_NF_CONNTRACK_TIMEOUT
...
{
	if (kernel_init_done == 1) printk("We reached unpopular paths in include/net/netfilter/nf_conntrack_timeout.h: line 103 \n"); 
        return 0;
}

static inline void nf_conntrack_timeout_fini(void)
{
	if (kernel_init_done == 1) printk("We reached unpopular paths in include/net/netfilter/nf_conntrack_timeout.h: line 109 \n"); 
        return;
}
